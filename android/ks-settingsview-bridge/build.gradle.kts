// ks-settingsview-bridge: KsSettingsView interop Bridge (Android)
//
...
// 既存の公開 API (`ks-settingsview-ui`) を interop 都合の型で汚染しないため、独立モジュールとする。

plugins {
    // Android Library プラグイン（ks-settingsview-core / ks-settingsview-ui と整合）
...
// JDK 17 を採用（core / ui モジュールと整合）
kotlin {
    jvmToolchain(17)
}

dependencies {
    // Core モジュール（SettingsRoot / Section / SectionAccessory / AccessoryTarget 等）
    implementation(project(":ks-settingsview-core"))

    // UI モジュール（SettingsRootStore / KsSettingsView / LabelCell / Theme）。
    // Bridge の公開 API は Bridge 自身の DTO と `android.view.View` だけを露出させ、
    // UI モジュールの型は輸送の内側に閉じ込める。
