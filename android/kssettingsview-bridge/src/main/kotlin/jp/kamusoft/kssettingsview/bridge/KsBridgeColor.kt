 ...
 * （maui/ADR-0004）。DTO 側の `null` は「未指定」を意味し、native 側の未指定表現
 * `Color.Unspecified` へ写して上位の既定（Theme 継承など）へ倒す。
 */
internal object KsBridgeColor {
     ...
     * @return 変換した色。未指定のときは `Color.Unspecified`
     */
    fun color(argb: Int?): Color = argb?.let { Color(it) } ?: Color.Unspecified
}
