...
上の 3 つは待ち方の作法であり、**何の成立を待つか**は別に選ぶ。条件には検証したい意味を直接表す値を選び、framework が内部の都合で切り替える値をその意味の代理に使わない。代理は「いつそう見えるか」の決定権を framework に預けることであり、実行機が遅いほど成立が遅れる。作法を 3 つとも満たした待機でも、条件の選び方だけで手元では常に緑・CI でだけ落ちる形になる。
