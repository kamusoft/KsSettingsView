...
| ドメイン | 内容 |
|---|---|
| [cross](cross/index.md) | リポジトリ横断の規約 (ソースコメント・テスト実行・実行時挙動の検証・Sample 一致・公開識別子・利用者向け Skill・インストール例・開発環境) |
| [ios](ios/index.md) | iOS 系統の規約 (Swift 6 言語モード適合の確認) |
| [maui](maui/index.md) | .NET MAUI 系統の規約 (検証ホストの実行・描画性能の計測構成) |

core / android のドメイン規約は現時点で存在しない。最初の文書を書くときに `<domain>/index.md` ごと作る。
