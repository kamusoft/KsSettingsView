...
- **変更パスによる絞り込み (2026-09-04)**: 引き続き行わない (cross/ADR-0025 を維持)。絞るのは事象 (どのブランチに何が起きたか) であって変更内容ではない
...
- [ ] change `slim-ci-triggers` を S 級として実装する ([exploration.md](../../../../changes/slim-ci-triggers/exploration.md) の「実装の見当」)
- [ ] handbook `cross/release-procedure.md` の「develop の保護設定が正」「必須 check 7 件」の記述を追従させる (change に同梱)
- [ ] 実装後、`develop` への push で 4 job だけが走ることを確認し、所要時間の実測をここに残す
