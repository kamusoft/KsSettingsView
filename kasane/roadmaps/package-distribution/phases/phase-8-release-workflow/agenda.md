...
- [ ] **phase-6 からの申し送り: NU1507 の扱い** (2026-09-02): `maui/Directory.Packages.props` で CPM を導入したため、NuGet ソースを 2 件以上構成した環境では `maui/` 配下全プロジェクトの restore で NU1507 (複数パッケージソース) が出る (単一ソースの環境では出ない。作業機で再現、証跡は [cpm-restore-invariance.txt](../../../../changes/add-maui-nuget-distribution/evidence/cpm-restore-invariance.txt))。release workflow のランナーが複数ソース構成になった場合も同じ警告が出るため、恒久対処を決める — 候補は (a) リポジトリに packageSourceMapping 付き `nuget.config` を置く (phase-7 の消費者検証がフィード限定の `nuget.config` を使う設計と重なる) / (b) `NoWarn` で明示的に受け入れる
- [ ] **phase-6 からの申し送り: XA4301 警告の扱い** (2026-09-02): 利用者アプリの Android Release ビルドで XA4301 が 4 件出る。.NET Android SDK が facade と Android binding の各 assembly に自動生成する自 assembly 用 aar (`KsSettingsView.Maui.aar` / `KsSettingsView.Binding.Android.aar`) が推移依存 `androidx.graphics.path` の ABI 別 `.so` を抱えており、APK 生成時に同じ `.so` と重複する。ビルドは成功し成果物への影響はない (証跡は [consumer-verification.txt](../../../../changes/add-maui-nuget-distribution/evidence/consumer-verification.txt))。公開前に (a) pack 時に自 assembly 用 aar を除外する設定を入れて解消する / (b) 利用者向け既知事項として README に記す、のどちらかを決める。deviation では facade / binding とも「SDK 挙動で混入ではない」と合意済み
