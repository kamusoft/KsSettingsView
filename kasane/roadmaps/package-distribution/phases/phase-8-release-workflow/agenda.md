...
- [ ] **phase-3 からの申し送り** (2026-08-31): `main` ブランチを作成するとき、作成と同時に branch protection を設定する。内容は `develop` と同じ — 検証 CI の 4 job (`ios / verify` / `android / verify` / `maui / verify` / `lint`) に phase-7 の消費者検証 3 job (`consumer-ios / verify` / `consumer-android / verify` / `consumer-maui / verify`。`develop` は 2026-09-02 に 7 job で設定済み — [branch-protection-develop.txt](../../../../changes/archive/2026-09-02-add-consumer-verification/evidence/branch-protection-develop.txt)) を加えて必須 status check とし、pull_request 経由を必須にする (承認数 0)。force-push 禁止・削除禁止を付け、admin バイパスは緊急時の逃げ道として許容する。phase-3 の決定事項「必須チェック化と通知」は `develop`・`main` 両方を対象としていたが、`main` が存在しないため `develop` のみで実施した ([phase-3 の実装結果](../phase-3-verification-ci/agenda.md))。設定は `gh api -X PUT` が保護設定を全体置換する点に注意し、既存設定を含む完全な payload を送る (実例: [branch-protection-develop.txt](../../../../changes/archive/2026-08-31-add-verification-ci/evidence/branch-protection-develop.txt))
- [ ] **phase-6 からの申し送り: NU1507 の扱い** (2026-09-02): `maui/Directory.Packages.props` で CPM を導入したため、NuGet ソースを 2 件以上構成した環境では `maui/` 配下全プロジェクトの restore で NU1507 (複数パッケージソース) が出る (単一ソースの環境では出ない。作業機で再現、証跡は [cpm-restore-invariance.txt](../../../../changes/archive/2026-09-02-add-maui-nuget-distribution/evidence/cpm-restore-invariance.txt))。release workflow のランナーが複数ソース構成になった場合も同じ警告が出るため、恒久対処を決める — 候補は (a) リポジトリに packageSourceMapping 付き `nuget.config` を置く (phase-7 の消費者検証がフィード限定の `nuget.config` を使う設計と重なる。2026-09-02 phase-7 決定: 消費者側は packageSourceMapping 付き `nuget.config` を採用し (a) と同型を先行実証する) / (b) `NoWarn` で明示的に受け入れる
- [ ] **phase-6 からの申し送り: XA4301 警告の扱い** (2026-09-02): 利用者アプリの Android Release ビルドで XA4301 が 4 件出る。.NET Android SDK が facade と Android binding の各 assembly に自動生成する自 assembly 用 aar (`KsSettingsView.Maui.aar` / `KsSettingsView.Binding.Android.aar`) が推移依存 `androidx.graphics.path` の ABI 別 `.so` を抱えており、APK 生成時に同じ `.so` と重複する。ビルドは成功し成果物への影響はない (証跡は [consumer-verification.txt](../../../../changes/archive/2026-09-02-add-maui-nuget-distribution/evidence/consumer-verification.txt))。公開前に (a) pack 時に自 assembly 用 aar を除外する設定を入れて解消する / (b) 利用者向け既知事項として README に記す、のどちらかを決める。deviation では facade / binding とも「SDK 挙動で混入ではない」と合意済み
- [ ] **phase-6 からの申し送り: `ContinuousIntegrationBuild` の注入** (2026-09-02): `maui/Directory.Build.props` は SourceLink の `PublishRepositoryUrl` / `EmbedUntrackedSources` / snupkg を持つが `ContinuousIntegrationBuild` は書いていない (ローカル pack で determinism 警告を出さないため)。release workflow の pack で `-p:ContinuousIntegrationBuild=true` を `-p:Version=` と併せて注入する。pack は csproj 単位で binding 2 件 → facade の順 (slnx は pack の単位ではない)
- [ ] **phase-7 からの申し送り 3 件** (2026-09-02): 下表「phase-7 からの申し送り」を論点の解消時に取り込む ([phase-7 の実装結果](../phase-7-consumer-verification/agenda.md))

### phase-7 からの申し送り (2026-09-02)

| 申し送り | 内容 |
|---|---|
| 消費者検証の呼び出し契約 | release workflow は `verify-consumer-{ios,android,maui}.yml` を publish 前に `mode: dry-run` + version + `artifact` (publish 段が upload した配布物。iOS はスナップショットで展開先末尾が `KsSettingsView-SPM`、Android は mavenLocal の `jp/` 配下、MAUI はフォルダフィード) で、publish 後に `mode: smoke` + version だけで呼ぶ (`smoke` + `artifact` は入力検査で拒否される)。artifact の upload 名は release 側で決める |
| `secrets: inherit` を使わない | 消費者 workflow は secrets を宣言しないが、呼び出し側が `secrets: inherit` を書くと全 secrets が渡る。release から呼ぶときに使わないことを必須確認事項にする |
| smoke 正ケースの実証 | 3 公開レジストリからの解決とビルド成功は初回リリースで初めて実証される。失敗時の扱い (再実行・tag の遅延) を job 構成の論点に含める |
| 所要時間 | consumer-maui が約 20 分 (workload install + 空の RestorePackagesPath で毎回フル restore)。release 全体の見積もりに入れる |
| 署名の生成確認 | `android/kssettingsview/build.gradle.kts` は署名鍵 (`signingInMemoryKey`) が無い発行をリリース版の version でも未署名で成功させる (消費者検証の dry-run がリリース版を扱うため)。「リリース版 × 鍵なし」の経路は PR CI では踏まれず自動担保が無いので、Maven publish の前に署名ファイル (`.asc`) の生成を確認するステップを置き、Central Portal の未署名拒否だけを歯止めにしない |
| docs-refresh 依頼の内容 | phase-5〜7 で溜まった skills / README の追随内容 ([phase-7 agenda の「docs-refresh 依頼の内容」](../phase-7-consumer-verification/agenda.md)) は、オーナーが先に docs-refresh を依頼しなければ初回リリース時の docs-refresh (「配信準備中」バナー解除) に併合する。concepts 側は蒸留済み (`maui/api/maui-facade.md` の要件表に API 版付き TFM の要件) |
