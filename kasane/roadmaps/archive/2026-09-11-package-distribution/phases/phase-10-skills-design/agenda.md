...
- **legacy-aiforms-reference.md の行き先 (2026-08-25)**: kasane/concepts/ の cross ドメイン配下へ移して開発資料 (歴史資料) として温存する。残りの移植作業 (maui-support pending フェーズ) と AiForms 移行 Skill の書き起こし材料の両方から参照できる。`kasane/handbook/cross/aiforms-origin-reference.md` と相互リンクし index に登載。正確なファイル名・カテゴリは phase-12 実装時に確定
