...
2026-09-15、settingsview-migration-defects の探索より。Pop されたページ自身の再 Push や Activity 再生成で Host が作り直されると、両 OS でスクロール位置が先頭に戻る (証跡: `kasane/changes/settingsview-migration-defects/evidence/*-sample-reconnect-*`。archive 後は媒体が削除されるので exploration.md の実測表を参照)。
