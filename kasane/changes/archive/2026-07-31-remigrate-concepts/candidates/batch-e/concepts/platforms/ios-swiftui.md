...
Section は文字列または `SectionAccessory` の Header / Footer、`headerHeight`、`isVisible` と Cell 列を受ける。`import SwiftUI` 時に `SwiftUI.Section` と名前が衝突する場合は、`KsSection` または `ksSection(...)` を使う。
...
同じ要素では、DSL 専用 `ForEach` の key と `sectionID(_:)` / `cellID(_:)` を併用しない。どちらか一方だけを identity として指定する。両方を組み合わせた優先順位は [ADR-0008](../../decisions/0008-stable-declarative-tree-identity.md) と現行 iOS 実装に drift があるためである。
