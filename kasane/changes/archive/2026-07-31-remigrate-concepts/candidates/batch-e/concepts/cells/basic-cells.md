...
`ButtonCell.titleAlignment` の既定は center で、Swift は `.start` / `.center` / `.end`、Kotlin は `START` / `CENTER` / `END` を使う。`valueText` / `icon` / `hintText` がなければ title は利用可能な行領域全体で alignment され、いずれかがあれば残った title 領域内で alignment される。補助フィールドの有無にかかわらず Disclosure Indicator は表示しない。
