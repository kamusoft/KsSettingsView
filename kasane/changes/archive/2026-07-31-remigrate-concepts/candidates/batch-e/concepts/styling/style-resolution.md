...
- icon size / radius が未指定なら、24pt / 24dp と radius 0へ解決する。ただし Android の現行共通行は解決済み icon size / radius を描画へ適用していないため、その反映を保証しない。
...
Theme は `SettingsRoot` の構造ではなく独立した表示状態である。同値 Theme は再適用せず、変更時は Section / Cell ID を維持したまま、各 platform が対応する表示属性を再評価する。iOS の現行 Host は canvas と表示中 Cell を更新する一方、すでに表示されている Header / Footer への即時再適用は保証しない。Android は行、背景、Section 装飾を再評価する。
