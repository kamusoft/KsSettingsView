...
基本 Cell は、設定行の表示時点の内容と状態を持つ UI 層の値モデルである。Native control の寿命や画面全体の状態は所有せず、利用者操作を callback で外部へ通知する。本書を Cell 共通契約の入口とし、入力固有の差分は [入力 Cell](cells/input-cells.md) で説明する。
...
全7種が `style`、`title`、`isEnabled`（既定 `true`）、`isVisible`（既定 `true`）を持つ。`ButtonCell` を除く6種は `description`、`valueText`、`icon`、`hintText` を持つ。`ButtonCell` は移植元との意味互換のため `description` を公開しない。

画像の case と fallback は [KsImage](cells/ks-image.md) を参照する。
