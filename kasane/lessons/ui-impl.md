...
ソースコメント規約 (`kasane/handbook/cross/comment-policy.md`) は UI 実装ワーカーの読み込み対象に入っておらず、規約の存在自体が届いていなかった。内容は `lessons/impl.md` の L-001 / L-002 と同一で、scope が開示の単位であるため両方に置く (片方を直したらもう片方も直す)。
