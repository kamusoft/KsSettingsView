...
| [0030](0030-theme-dark-appearance-library-owned-light-dark-defaults.md) | Theme のダーク対応はライブラリ所有の light / dark 既定値と未指定色の描画時解決で行う | proposed | 既定色を全部外観に追随 (light は現行値、dark はライブラリ所有の生値を 3 面同値)。未指定色だけ描画時に解決し明示指定は変えない。Theme 型は不変で対の型・コールバックは作らない。Android は `Color.Unspecified` + Defaults factory の 2 セット、values-night は置かない。 |
