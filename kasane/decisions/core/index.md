...
| [0030](0030-theme-dark-appearance-library-owned-light-dark-defaults.md) | Theme のダーク対応はライブラリ所有の light / dark 既定値と未指定色の描画時解決で行う | accepted | 既定色を全部外観に追随 (light は現行値、dark はライブラリ所有の生値を 3 面同値。iOS の title / description / ButtonCell はシステム色のまま)。未指定色だけ描画時に解決し明示指定は変えない。Theme 型は不変で対の型・コールバックは作らない。Android は `Color.Unspecified` + `KsSettingsViewDefaults` の 2 セット (title / description は factory に載せず実効 style の最終段で外観から選ぶ)、`DEFAULT_*` 色定数は削除、values-night は置かない。View は uiMode 変更で未指定色を再解決する。一部改訂: [0031](0031-explicit-cell-color-appearance-contract-and-beta-breaking-change.md) (未配信の前提・Decision 3 の MAUI 節)。 |
| [0031](0031-explicit-cell-color-appearance-contract-and-beta-breaking-change.md) | 未指定色の Unspecified 統一と明示色の外観契約を Cell 固有色まで広げ、beta 配信中は互換経路なしの破壊的変更を受容する | accepted | ADR-0030 を一部改訂 (amends)。Android の Cell 固有色 12 本を `Color.Unspecified` の 1 流儀へ、明示した CellStyle 色 / Cell 固有色もライブラリは置き換えず手段は platform 慣用 (iOS dynamic `UIColor` / Compose 構築時分岐 / Store `replaceCell` / MAUI は Theme プロパティのみ `AppThemeBinding`、Cell プロパティは `RequestedThemeChanged` 購読 + 再代入)。beta 配信中は互換経路なしの破壊的変更を受容。bridge DTO に CellStyle 段の placeholder は足さない。 一部改訂: [cross/0032](../cross/0032-maui-section-cell-as-logical-children.md) (Decision 2 の MAUI 節 — Section / Cell を論理子にし、Cell のプロパティも `AppThemeBinding` で追随)。 |
| [0032](0032-header-footer-background-default-transparent.md) | Header / Footer 背景色の既定は両外観とも透明にする | proposed | core/ADR-0030 の既定値のうち Header / Footer 背景だけを置き換える (amends 0030)。iOS にも背景色を反映するにあたり、既定では帯を出さず list 下地を見せる (AiForms 互換)。iOS の公開定数は両外観の値を持つ形のまま値だけ透明。 |
