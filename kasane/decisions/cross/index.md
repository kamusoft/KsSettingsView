...
| [0019](0019-lockstep-single-version.md) | 全 platform を lockstep の単一バージョンでリリースする | accepted | 全 artifact に同一 semver (prerelease は `X.Y.Z-{alpha|beta|rc}.N`) を付け git tag と一致させる。互換マトリクスは作らない。初回 `0.1.0-beta.1` で実証。 |
| [0020](0020-release-dispatch-tag-last-version-injection.md) | リリースは version 入力の手動起動で行い、全 platform の publish 成功後に tag を打ち、version は CI が注入する | accepted | tag 先行で lockstep が壊れるのを防ぐ。version の SSoT は dispatch 入力 (= tag)、ファイルは開発用既定値。起動は `main` からのみ、secrets は Environment `release`。publish は冪等で同じ version から再実行できる。 |
...
| [0022](0022-user-docs-as-agent-skills.md) | 利用者向けドキュメントは Agent Skills (skills/、en/ja 2 版) として提供 | accepted (supersedes 0014、一部改訂: 0029) | docs/ を廃止し platform 別 + 移行の 4 Skill × 2 言語をコピー利用で提供。知識の正は concepts、manifest の逆引きと網羅検査で追従。Skill は単体コピーの閉世界で自己完結させる。初期生成・構成変更は変更フロー、docs-refresh は追従専用。 |
