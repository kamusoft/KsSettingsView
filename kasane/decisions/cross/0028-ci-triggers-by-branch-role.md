...
整理: 2026-09-07 実装後の実測 (`develop` push の壁時計 7.7 分の内訳) と、除外の起動条件を run 一覧で確認した記録を Consequences から除いた。起動条件そのものは同節の「開発ハーネスの記録だけの push では CI が起動しない」が述べており、所要時間の実測は腐りやすく決定の帰結ではない。決定内容 (Context / Decision / Alternatives) は不変
...
現行照合: 2026-09-07 確認。`.github/workflows/ci.yml` の `on:` は `pull_request` が `main` 宛てのみ、`push` が `develop` のみで、`paths-ignore` に `kasane/**` と Issue テンプレート・CONTRIBUTING を持つ。`develop` 宛ての pull_request トリガーは無い。判定: 維持
