...
tags: [sample, conventions, verification]
timestamp: 2026-08-29
---

# Sample のプラットフォーム間一致

この文書は、`samples/` 配下の Sample アプリをプラットフォーム間でどこまで一致させるかの規約を定める。読むと、Sample に画面や文言を追加・変更するときに何を揃えなければならないか、どんな差異なら許されるかが分かる。[リポジトリとビルドの責務境界](../architecture/repository-boundaries.md) を先に読むと、Sample が何を保証する装置かが分かりやすい。
...
- **表示文言の完全一致**: 画面タイトル・メニュー項目・Section の header / footer・Cell の表示フィールド (title / description / hintText 等 — [基本 Cell](../../core/cells/basic-cells.md) 参照)・デモデータ (初期値・選択肢の文言) は一字一句一致させる。大文字小文字の違いや「〜デモ」の有無のような表記ゆれも不一致とみなす
...
- この規約を製品契約と混同しない。[リポジトリとビルドの責務境界](../architecture/repository-boundaries.md) の通り、Sample の表示文字列やデモデータは利用者向けの製品契約ではない (変更しても breaking change ではない)。本規約は「platform 間で互いに揃える」内部の検証規約であり、両者は別軸で両立する

## 関連

- [リポジトリとビルドの責務境界](../architecture/repository-boundaries.md) — Sample が利用者側から本体を参照する境界と、製品契約にしない範囲
