...
builder の `section(...)` は core の `Section` data class と同じ引数・同じ既定値を持つ (accessory 版と文字列ヘッダ版の 2 オーバーロード。`headerHeight`・`isVisible`・表示トグル `isHeaderVisible` / `isFooterVisible` を受け、文字列ヘッダ版は文字列 `footer` も受ける)。`Section` で表せる構成は builder を諦めずにそのまま書ける。

## DSL の構築 API

`Section(...)` は文字列または任意 Composable の Header / Footer、`headerHeight`、`isVisible`、Header / Footer の表示トグル `isHeaderVisible` / `isFooterVisible` (core/ADR-0023) と Cell 列を受け、`SectionHandle` を返す。同じ位置に文字列と Composable を同時指定すると失敗する。
