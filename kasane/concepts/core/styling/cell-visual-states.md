...
**例外: CustomCell** — content が任意の宣言 UI であり、ライブラリ側から text 要素を特定して意味色を置換できないため、content 全体の淡色化 (alpha 0.38。`Theme.disabledTextColor` #999999 が白背景上で持つ濃度に合わせた値) で無効を表現する。行背景と Disclosure Indicator は対象外。iOS では SwiftUI 標準コントロールの `.disabled` による淡色化と重なり二重に薄くなるが、両プラットフォームで「無効に見える」ことを優先した合意済みの差分。無効時の accessibility はプラットフォーム非対称 (Android は TalkBack の読み上げ対象から外れる — [core/ADR-0017](../../../decisions/core/0017-customcell-disabled-suppression-over-a11y-symmetry.md))。詳細は [CustomCell](../cells/custom-cell.md) を参照。
...
- disabled 表現を text と Native control へ適用し、行全体を一律に薄くしない (CustomCell は無効状態の節の例外に従う)。
- Cell 固有の意味色を通常の style 解決より先に扱う。

## してはいけないこと

- 無効 Cell に押下・選択 feedback を残さない。
- 行全体の alpha だけで disabled 状態を表現しない (共通行構造を持つ Cell の規則。full-bleed の CustomCell は上記の例外に従う)。
