...
- [styling/style-resolution.md](styling/style-resolution.md) — UI 層が Theme と CellStyle を所有し、platform の描画値へ段階的に解決する共通規則。ライブラリ所有の light / dark 既定色と未指定色の外観解決 (core/ADR-0030)、明示した CellStyle 色 / Cell 固有色の外観契約と platform 別の切替手段 (core/ADR-0031)、ライブラリ既定色を中立に保つ方針、iOS のモーダル提示物が提示元の外観を引き継ぐ契約を含む
