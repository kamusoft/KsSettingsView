...
確定操作では、その時点の選択 (時・分・12 時間制では午前/午後) から作った値を引数に `onValueChanged` を1回発火して閉じる。Android の確定値は `LocalTime`、iOS は元の `cell.time` の日付成分を保持した `Date` である。

非確定の閉じ方 (キャンセル・外側タップ・Back・下スワイプ等、器が提供するすべての経路) では callback を発火せず、変更を破棄する。
...
Android は Host Context の現在 Locale を提示時と Cell の bind 時に使い、構成変更で表示中 Cell を再 bind する。iOS は表示中に Locale 変更通知が届いた場合も、選択面と自動 `valueText` の Locale だけを更新する。利用者がまだ確定していない時刻は保持し、Locale 変更を値変更として通知しない。
...
- `is24Hour = false`: 12 時間制 — 時 1–12 / 分 0–59 / 午前・午後 の3系列
- 午前/午後のラベルは表示 Locale の表記から導出する (自前の翻訳文字列は同梱しない)

12 時間制の系列順は表示 Locale の表記パターンに従う。ja 等の前置き Locale では午前/午後・時・分、en 等では時・分・AM/PM となり、iOS の埋め込み picker が Locale で列順を組み替える挙動と揃う。深夜 0 時台は「12 + 午前」、正午台は「12 + 午後」として提示・往復する。

iOS の埋め込み picker も時制は `is24Hour` で決まる。強制は hour cycle のみに作用させ、AM/PM 等の**表記の言語・地域は表示 Locale 由来を保つ** (`HourCycleLocale` が基準 Locale の hourCycle だけを差し替える)。
...
- 選択面と自動 `valueText` は同じ表示 Locale を使い、実行中の Locale 更新で未確定の時刻を失わない (core/ADR-0035)
