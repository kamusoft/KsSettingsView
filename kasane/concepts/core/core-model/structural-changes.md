...
target と payload 種別の一致は呼び出し側の事前条件である。Section target の ID の存在は事前条件ではない — 未知の sectionID を渡した `updateAccessory` / `UpdateAccessory` の Store 公開操作は、state 更新も Diff 発行も行わない no-op になることを Store が両 OS 対称に保証する (core/ADR-0020)。Root target は Store に state を持たないためこの判定の対象外で、常に発行される。`full` / `Full` では Root Accessory は変わらず、Root target の `updateAccessory` / `UpdateAccessory` を適用したときだけ UI 層の保持値が変わる。
