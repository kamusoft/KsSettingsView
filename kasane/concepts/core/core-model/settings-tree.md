...
| `Section` | ID、Section Header / Footer、順序付き Cell、`headerHeight`、`isVisible`、`isHeaderVisible` / `isFooterVisible` を保持する |
...
`Section.isHeaderVisible` / `isFooterVisible` は既定 `true` の表示トグルで、`false` にすると Header / Footer を内容を保持したまま隠す。表示は「トグル && 内容あり」の AND で判定するため、内容が無いものをトグルで表示させることはできない (core/ADR-0023)。
...
- Header / Footer の内容が不在 (未設定、または空 text。view は常に内容あり) のとき、UI 層は表示領域を作らない。`Section.headerHeight` / `Theme.headerHeight` の高さ指定は存在する Header の高さを決めるだけで、Header の存在を作らない (core/ADR-0023)。
- `isHeaderVisible = false` / `isFooterVisible = false` の間も accessory の値は model に保持され、非表示中の内容更新は `true` へ戻した再表示に反映される。
- `Section.isVisible = false` の Section は model に保持され、visible projection から Header / Footer / Cell ごと除外される。
- `SettingsRoot` と `Section` は値比較でき、`Section` の比較には ID、Accessory、Cell、`headerHeight`、`isVisible`、`isHeaderVisible`、`isFooterVisible` が参加する。
