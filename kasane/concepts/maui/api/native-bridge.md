...
| gateway | facade 内部の `KsBridgeGateway` (per-TFM = ターゲットフレームワーク net10.0-ios / net10.0-android ごとの実装) | facade と Binding の間に立つ薄い C# 変換層。facade と同寿命で、facade 型 ⇔ DTO の変換と delegate / listener 実装の保持を担う (facade 内部実装であり公開契約ではない) |
...
| メソッド | 対象 | 意味 |
|---|---|---|
| `setRoot` | Root | 設定ツリー全体の置換 |
| `insertSection` / `removeSection` / `moveSection` / `replaceSection` | Section 構造 | Section の構造操作 4 種 |
| `insertCell` / `removeCell` / `moveCell` / `replaceCell` | Cell 構造 | Cell の構造操作 4 種 |
| `replaceCells` | Cell 内容 | 複数 Cell の内容更新を1バッチで適用 |
| `updateAccessory` | accessory | root または指定 Section の header / footer テキストの更新と clear。`target` 引数 (root / section × header / footer の 4 値) と `sectionID` で対象を選ぶ |
| `updateAccessoryView` | accessory (view) | accessory への任意 platform view (iOS `UIView?` / Android `View?`、null でクリア) の設定。Store の accessory 経路は view ではなくそれを組み立てる closure (builder) を受けるため、Bridge 内部で常に同じ実体を返す定数返し closure に包み (返す前に既存親から detach)、Store の `updateAccessory` 経路に合流する (maui/ADR-0017) |
...
双方向値の輸送は maui/ADR-0012 の規約に従う: Bool / Int / String は素通し、Picker の選択は index (複数選択は昇順・重複除去の Int 配列)、Time / Date は壁時計値の ISO-8601 文字列 ("HH:mm" / "yyyy-MM-dd")。

Picker の候補は per-item DTO (`KsBridgePickerItem`: 主表示 `text` + nullable `subText`) の列で運ぶ — 平行配列 (`texts` + `subTexts`) にしない。件数不一致と null 要素の binding 表現という不変条件が型で閉じ、Objective-C / .NET binding では nullable string プロパティとして表現できる (core/ADR-0029)。

Optional を型で表せない scalar (uiStyle の enum 序数等) は「未指定を表す値」(負値センチネル等) を DTO ごとに定める (個々の表現は実装とテストが正)。DTO の未指定時の既定値は native 既定のリテラル複製ではなく native インスタンスからの導出で持つ — リテラルを複製すると native 側の既定が変わっても DTO 側が古い値のまま残り、誰も気づけないため。
...
DTO (`KsBridgeSection` / `KsBridgeLabelCell` 等) はインスタンス生成時に自分でも ID を採番して公開するが、Store 上の identity になるのは insert / Builder 経由で追加された時点の ID だけである。`replaceSection` / `replaceCell` は**対象の既存 ID を維持したまま**内容を作り直すため、渡した DTO 自身の `sectionID` / `cellID` は Store のどこにも存在しない ID になる — DTO が公開する ID を後続操作に使ってはいけない。`replaceSection` / `replaceCell` の戻り値は置換後に有効な ID (= 対象の ID)。対象が見つからず no-op になった場合は null を返す。
...
binding (iOS xcframework / Android aar の生成と取り込み、SDK 標準アイテムの採否、既知の制約、SDK 更新時の再検証箇所、NuGet 3 パッケージの pack 構成) の正は [MAUI binding の Native artifact 統合](../architecture/binding-build-integration.md) が持つ。`maui/tests/` の検証用ホストアプリ (iOS / Android) で C# から Native 表示までの end-to-end 疎通を確認する手順は [MAUI 検証ホストの実行規約](../../../handbook/maui/integration-host-verification.md)。

## 関連

- [MAUI facade (KsSettingsView.Maui) の公開契約](maui-facade.md) — この interop 境界の上に載るアプリ向け公開面 (Cell・スタイル・表示への反映と Host の寿命はそこから辿る)
