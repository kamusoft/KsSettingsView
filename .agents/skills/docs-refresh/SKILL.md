...
**最初にサブエージェント委譲機構が使えるかを確認する。** 具体的には「器 `ksn-scribe` を指定してサブエージェントを起動できるか (器が実行環境に配置済みか)」を確認する。確認の手段は、利用可能なサブエージェント種別の一覧に `ksn-scribe` があること、または実行環境の器定義ファイルの存在確認 (Claude Code: `~/.claude/agents/ksn-scribe.md` か `.claude/agents/ksn-scribe.md` / codex: `~/.codex/agents/ksn-scribe.toml`) のいずれかでよい (片方が成立すれば配置済みとみなす)。使えない場合は、以降の手順 (Step 2 以降) へ進まず、何も書き換えないまま実行不能として停止する (「コンテキスト節約方針」)。

器が未配置の場合は、次の趣旨を案内して停止する (メインが代わりに書くフォールバックも、器なしのサブエージェント起動もしない):

```
サブエージェントの器 ksn-scribe がこの環境に配置されていません。
...
**サブエージェントの起動は常に器 `ksn-scribe` の指定付きで行う** (Claude Code: `subagent_type: ksn-scribe` / codex: エージェント name `ksn-scribe` で spawn)。器を指定しない起動はしない (メインのモデルを継承し、編成の切り離しが成立しなくなる。「コンテキスト節約方針」)。器が未配置なら Step 1 で停止しているはずである。
...
どちらのテンプレートも先頭にコンテキストパッケージ節を持つ。器 `ksn-scribe` は「渡されたコンテキストパッケージの読むべきスキルを読み、パッケージなしで起動されたら作業しない」と定められているため、この節を削って渡さない。
...
- **器の指定なしでサブエージェントを起動しない**。サブエージェントは常に器 `ksn-scribe` を指定して起動し (Claude Code: `subagent_type` / codex: エージェント name)、器が未配置なら Step 1 で停止する (器なしの起動はメインのモデルを継承し、編成の切り離しが成立しなくなる)
