...
本 Requirement における関数名 `applyCellBaseLayout` は、`unify change` の旧版で採用されていた関数名 `ksCellRow` から **リネーム** されたものである。`ksCellRow` 関数は本 Requirement 適用時に **削除** しなければならない (MUST)。両者の I/F は等価であり、シグネチャの第 1 引数は引き続き `UICollectionViewListCell` を受け取る UIKit Builder 関数である。
