...
- **Section 構成** — iOS Sample と同等の 7 セクション構成（CommandCell プロフィール風 / LabelCell Storage 風 / SwitchCell + CheckboxCell / RadioCell 2 件 / SimpleCheckCell 複数 / ButtonCell / 任意 1 セルで HintText）。
...
- **WHEN** 起動して画面を観察する
- **THEN** 全体の背景がベージュ系（`#F2EFE6`）、セル背景が白、セクションヘッダ文字が黄系（`#CC9900`）、SwitchCell の ON 時 thumb / track と CheckboxCell の Checked 時のボックスが `#FFBF00` 系で表示される
...
- **WHEN** 描画される
- **THEN** 各アクセサリ要素の右端 X 座標が一致する（CheckboxCell が他より内側にずれない）
...
- **WHEN** 表示される
- **THEN** 当該 Cell の所定位置に HintText が表示される
