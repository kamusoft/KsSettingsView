...
fun InputCellsDemoScreen() {
    ...
    val prefectureIndex = remember { mutableStateOf<Int?>(12) }
        ...
        KsSettingsView(
            ...
            Section(header = "PickerCell（単一選択）") {
                ...
                PickerCell(
                    title = "都道府県",
                    items = prefectures,
                    selectedIndex = prefectureIndex.tracked(
                        title = "都道府県",
                        display = { index -> prefectures.getOrNull(index ?: -1) ?: "(未選択)" },
                        onEvent = record,
                    ),
                    pageTitle = "都道府県を選択",
                )
