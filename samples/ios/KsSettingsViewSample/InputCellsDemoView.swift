...
struct InputCellsDemoView: View {
    ...
    /// 単一選択：都道府県 index（候補が画面に収まらない長さの選択面のデモ）
    @State private var prefectureIndex: Int? = 12
            ...
            KsSettingsView {
                ...
                Section("PickerCell（単一選択）") {
                    ...
                    PickerCell(
                        title: "都道府県",
                        items: prefectures,
                        selectedIndex: tracked($prefectureIndex, title: "都道府県", display: { idx in
                            guard let idx, prefectures.indices.contains(idx) else { return "(未選択)" }
                            return prefectures[idx]
                        }),
                        pageTitle: "都道府県を選択"
                    )
                }
