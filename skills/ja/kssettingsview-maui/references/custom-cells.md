...
`IsEnabled="False"` は Cell のタップと内容の中の操作の両方を抑止し、内容全体を淡色化する。Android では無効化した内容は TalkBack の読み上げ対象からも外れる。
...
`HasUnevenRows="True"` では内容を自動計測した高さが Cell の最低高になり、開閉するブロックや折り返すラベルを置くと表示中も行が伸びる。固定行にする場合は内容が収まる `Height` を選ぶ — 固定行が内容に合わせて伸びることは保証されない。初回から利用可能な幅を使って計測するので、折り返す内容も最初のフレーム後に遅れて伸びるのではなく、計測済みの高さで始まる。手動での再計測は要らない。
