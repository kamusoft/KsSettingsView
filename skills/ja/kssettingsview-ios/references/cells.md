...
設定画面に行を置くためのレシピ。例はいずれも [SKILL.md](../SKILL.md) の最小動作コードと同じ import を前提とする。例には UIKit の型がそのまま現れる (`titleColor` の `UIColor`、`keyboardType` の `UIKeyboardType`)。`import SwiftUI` から UIKit が入らないファイルでは `import UIKit` が要る。

## 行を Section にまとめる

行は必ず Section の中に置く。`ksSection` は文字列の Header と Footer を任意で受ける。
...
`ButtonCell` は Disclosure Indicator を表示せず、title を既定で中央寄せにする。`titleAlignment` は `CellTitleAlignment` (`.start` / `.center` / `.end`) を受け、視覚に出るのは `valueText` を持たない行だけ。
...
同じ `groupId` を持つ `RadioCell` が 1 つの選択グループになる。`value == selectedValue` の行が選択表示になり、`selectedValue` は利用者が所有する。
...
`PickerCell` は行タップで選択画面を開く。単一選択は `selectedIndex` を使い、候補をタップした時点で callback が 1 回発火して画面が閉じる (別途の確定操作は無い)。キャンセルで閉じた場合は、単一選択・複数選択のどちらも発火しない。
...
`TimePickerCell` は `Foundation.Date` の時刻部分を編集し、`format` が行に出る文字列を決める。
...
選択 UI の時制 (12/24 時間制) は `is24Hour` だけで決まる (既定 `true` = 24 時間制)。`format` は行に出る文字列にしか効かず、端末の 24 時間設定も参照されない。12 時間制にするなら `is24Hour: false` と、それに合う `"h:mm a"` のような `format` を組で渡す (両者の食い違いをライブラリは検証しない)。`pickerTitle` は時刻選択画面のタイトルを上書きする (未指定なら `title` を使う)。
...
組み込み Cell は `description` (title の下)、`valueText` (title と同じ行の末尾)、`hintText` (行の右上) を受ける。例外は 2 つで、`ButtonCell` は `description` を持たず、`EntryCell` は入力欄自身が値を表示するため `valueText` を持たない (`text` を使う)。
...
行幅が足りないときは title を守り、`valueText` 側が末尾省略で切り詰められる。

## 行を無効にする

構築時に `isEnabled: false` を渡す。タップと内包コントロールの操作が無効になり、文字色が無効時の色に置き換わる。Cell 自身にも `.disabled(_:)` modifier があるが (SwiftUI の `View.disabled(_:)` とは別物)、これは no-op なので行は無効にならない。
...
`isVisible: false` は行を表示から外すが値は model に残るため、隠れている間に適用した更新は再表示時に現れる。
...
`isVisible` は `ksSection` にもあり、その Section の Header・Footer・全行をまとめて隠す。
